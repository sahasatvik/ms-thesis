...
one of $k$ populations, described by distributions $F_i$ for $i \in \N_k$.
The populations may also be associated with prior probabilities $\pi_i$.

\begin{definition}[Classifier]
    A classifier is a map $\hat{i}\colon \R^d \to \N_k$.
\end{definition}

\begin{example}[Bayes classifier]
    Suppose that the population densities $f_i$ for each $i \in \N_k$ are
    known.
    The Bayes classifier assigns $\vx$ to the $\hat{\iota}_B$-th population
    where
    \begin{equation}
        \hat{\iota}_B(\vx) \,=\, \argmax_{i \in \N_k}\; \pi_i f_i(\vx).
    \end{equation}
\end{example}
...
\begin{example}[Maximum depth classifier]
    ...
    \begin{equation}
        \hat{\iota}_D(\vx) \,=\, \argmax_{i \in \N_k}\; D(\vx, F_i).
    \end{equation}
\end{example}
...
\begin{equation}
    \mathscr{D} \,=\, \{(\vx_{ij}, i)\} \subset \R^d \times \N_k,
\end{equation}
where $\vx_{i1}, \dots, \vx_{in_i} \iid F_i$ for each $i \in \N_k$.
...
\begin{equation}
    \hat{\iota}_{D, \mathscr{D}}(\vx) \,=\, \argmax_{i \in \N_k}\; D(\vx, \hat{F}_i).
\end{equation}
...
\begin{theorem}[\cite{ghosh-chaudhuri-2005}]
    ...
    $\Delta(\hat{\iota}_{D, \mathscr{D}}) \to \Delta_B$ as $\min\{n_1, \dots,
    n_k\} \to \infty$.
\end{theorem}
...
\begin{definition}
    ...
    \begin{equation}
        \Delta_{\mathscr{D}}(\hat{\iota}) \,=\, \sum_{i = 1}^k \frac{\pi_i}{n_i}\sum_{j = 1}^{n_i} \bm{1}(\hat{\iota}(\vx_{ij}) \neq i).
    \end{equation}
\end{definition}
...
\begin{definition}[DD classifier]
    ...
    \begin{equation}
        \hat{r} = \argmin_{r' \in \Gamma} \Delta_\mathscr{D}(\hat{\iota}_{D, r', \mathscr{D}}).
    \end{equation}
\end{definition}
\begin{remark}
...
\begin{lemma}
    ...
    \begin{enumerate}[itemsep = -0.2em]
        ...
        \end{equation}
    \end{enumerate}
    Then, $\Delta(\hat{\iota}_{D, \hat{r}, \mathscr{D}}) \to \Delta_B$ as
    $\min\{n_1, n_2\} \to \infty$.
\end{lemma}
...
\begin{equation}
    \mathscr{D}^D \,=\, \{(\vx^D_{ij}, i)\} \subset [0, 1]^k \times \N_k
\end{equation}
